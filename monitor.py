...
def fetch_klines(tf, limit=1000):
    ...
    # 4h周期需要更多数据用于EMA计算，且可能更容易超时，增加重试
    is_long_tf = tf in ['1h', '4h']
    max_retries = 2 if is_long_tf else 1
    ...
    for attempt in range(max_retries + 1):
        for name, source_fn in _DATA_SOURCES:
            try:
                ...
                if klines and isinstance(klines, list) and len(klines) >= 20:
                    ...
                    _source_health[name] = {'status': 'partial', 'last_success': time.time(), 'fail_count': 0}
            except Exception as e:
                _source_health[name] = {
                    'status': 'error',
                    'last_success': 0,
        ...
        if attempt < max_retries:
            wait_time = 1.5 if is_long_tf else 0.5
            logger.info(f"[{tf}] 第{attempt+1}次尝试失败，{wait_time}秒后重试...")
            time.sleep(wait_time)
    
    logger.error(f"[{tf}] ❌ 所有数据源均获取失败")
    _source_health['overall'] = {'status': 'offline', 'last_check': time.time()}
    return None
...
def _should_push_price_now(cfg=None):
    """判断是否到了推送时间：距离上次推送 >= 配置的间隔小时数。"""
    if cfg is None:
        cfg = load_config()
    interval_hours = cfg.get('feishu', {}).get('price_push_interval_hours', 4)
    now_ts = time.time()
